...
public static class PodManager
{
    ...
    public static AssetBundle BundleHandle
    {
        get
        {
            _bundleHandle ??= EmbeddedUtilities.LoadEmbeddedAssetBundle(
                Melon<Entrypoint>.Instance.MelonAssembly.Assembly, $"{EmbeddedAssetsPath}.limesfashionpods.bundle");
    ...
    private static IdentifiableType GenerateFashion(FashionParams p)
    {
        ...
        if (p.AttachPrefab != null) newAttach = Object.Instantiate(p.AttachPrefab);
        ...
        if (fashionRemover != null) fashionRemover.RemoveFX = SRLookup.Get<GameObject>("FX FashionApply");
    ...
    private static GadgetDefinition GeneratePodGadget(PodParams p)
    {
        ...
        newGadgetDef._localizedDescription =
            TranslationPatcher.AddTranslation(TableName, p.InternalName + "_desc", p.Description);
        ...
        newGadgetDef.Categories = new Il2CppSystem.Collections.Generic.List<IdentifiableCategory>();
