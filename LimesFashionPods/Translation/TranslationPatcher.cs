...
{
    private static readonly Dictionary<string, Dictionary<string, string>> AddedTranslations = new();
    ...
    public static LocalizedString AddTranslation(string table, string key, string localized)
    {
        ...
        if (patched.ContainsKey(key))
        {
            Entrypoint.Logger.Msg(
                $"Translation Key {key} for table {table} is already taken by another mod! Overwriting");
            patched[key] = localized;
            var localizedStringTableEntry = stringTable.GetEntry(key);
            localizedStringTableEntry.Value = localized;
            return new LocalizedString(stringTable.SharedData.TableCollectionName,
                localizedStringTableEntry.SharedEntry.Id);
        }
